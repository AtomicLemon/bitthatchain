...
*Setup instructions for a Gitian build of bithatchain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the bithatchain
...
- [Building bithatchain Core](#building-bithatchain-core)
...
Clone the git repositories for bithatchain Core and Gitian.
...
in the bithatchain Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building bithatchain Core
----------------

To build bithatchain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bithatchain Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bithatchain/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bithatchainpay/bithatchain
...
For example:
```bash
URL=https://github.com/crowning-/bithatchain.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bithatchain=${COMMIT} --url bithatchain=${URL} ../bithatchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bithatchain=${COMMIT} --url bithatchain=${URL} ../bithatchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bithatchain=${COMMIT} --url bithatchain=${URL} ../bithatchain/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bithatchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bithatchain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bithatchain
SIGPATH=/some/root/path/bithatchain-detached-sigs

./bin/gbuild --url bithatchain=${BTCPATH},signature=${SIGPATH} ../bithatchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bithatchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bithatchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bithatchain-osx-build.assert
```
...
[bithatchain/gitian.sigs](https://github.com/bithatchainpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
