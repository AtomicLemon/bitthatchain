...
const char * const BITCOIN_CONF_FILENAME = "bithatchain.conf";
const char * const BITCOIN_PID_FILENAME = "bithatchaind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bithatchain" is a composite category enabling all bithatchain-related debug output
                if(ptrCategory->count(std::string("bithatchain"))) {
...
#else
    const char* pszModule = "bithatchain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bithatchain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bithatchain
    // Mac: ~/Library/Application Support/bithatchain
    // Unix: ~/.bithatchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bithatchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bithatchain";
#else
    // Unix
    return pathRet / ".bithatchain";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bithatchain.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bithatchain.conf
