...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in bithatchain.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("bithatchain.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "bithatchain",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "bithatchain",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "bithatchain",               "gobject",                &gobject,                true,  {} },
    { "bithatchain",               "voteraw",                &voteraw,                true,  {} },
